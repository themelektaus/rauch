...
- `rauch.csproj`: .NET 10.0 project, excludes `plugins/` from compilation, generates `Usings.g.cs` during build
...
7. **Namespace**: Commands in `Rauch.Commands.Standalone`, groups in `Rauch.Commands.<GroupName>`, plugins in `Rauch.Plugins.<GroupName>`
