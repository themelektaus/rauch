...
- Sounds are embedded as resources from `Sounds/*.wav` with companion `*.wav.ini` config files
...
- Sound configuration loaded from INI files at startup via `LoadSounds()`
...
**Sound Configuration** (`Sounds/*.wav.ini`):
Each sound has a companion INI file with properties:
```ini
[Properties]
Volume=0.8
Duration=0.5
...
- `Duration`: Sound duration in seconds (optional, uses actual length if not specified)

**Available Sounds**:
- `Success`: Success completion sound (used by `logger.Success()`)
- `Error`: Error sound (used by `logger.Error()`)
- `Warning`: Warning sound (used by `logger.Warning()`)
...
**Usage in Commands**:
```csharp
SoundPlayer.Play("Success");  // Play success sound (fire-and-forget)
...
- `rauch.csproj`: .NET 10.0 project with unsafe blocks enabled, copies `Plugins/` to output, embeds `Sounds/*.wav` and `Sounds/*.wav.ini` as resources, generates `Usings.g.cs` during build, includes `Logo.ico` as application icon
...
- `Sounds/*.wav.ini`: Sound configuration files with Volume and Duration properties
- `Sounds/Excluded/`: Directory for excluded sounds (not embedded)
...
11. **Sound Feedback**: Use `SoundPlayer.Play(name)` for audio feedback (fire-and-forget, no await needed):
    - `SoundPlayer.Play("Success")`: After successful operations (also called automatically by `logger.Success()`)
