...
**rauch** is a .NET 10.0-windows console application with a reflection-based command architecture featuring automatic command discovery, validation, async execution, dependency injection, a powerful runtime plugin system with C# 13 compilation, interactive console UI components, and audio feedback via embedded sound effects.
...
- Static class for playing embedded WAV sound effects
- Sounds are embedded as resources from `Sounds/*.wav`
- Plays sounds asynchronously in background tasks
- `SoundPlayer.Wait()` waits for all sounds to finish before program exit
...
- `SoundPlayer.PlayLevelUp()`: Level up/achievement sound
- `SoundPlayer.PlayError1/2/3()`: Various error sounds
- `SoundPlayer.PlayNope()`: Rejection/invalid action sound
- `SoundPlayer.PlayReject()`: Access denied sound
- `SoundPlayer.PlayWhip()`: Quick action sound

**Usage in Commands**:
```csharp
await SoundPlayer.PlaySuccess();  // Play success sound
await SoundPlayer.PlayError1();   // Play error sound
```
...
- `rauch.csproj`: .NET 10.0-windows project with unsafe blocks enabled, copies `Plugins/` to output, embeds `Sounds/*.wav` as resources, generates `Usings.g.cs` during build, includes `Logo.ico` as application icon
...
**Framework**:
- .NET 10.0-windows (Windows-only due to SoundPlayer and PowerShell dependencies)
...
11. **Sound Feedback**: Use `SoundPlayer` methods for audio feedback:
    ...
    - `await SoundPlayer.PlayError1/2/3()`: For error conditions
    - `await SoundPlayer.PlayEnter()`: For confirmations
    - Fire-and-forget: `_ = SoundPlayer.PlayWhip()` (no await needed)
